"""
Train sequence prediction model using extended vocabulary. 
Use the pre-aligned embeddings. Reach 4.9% recall@5 

So using pre-aligned embeddings does help, to move from 4.7% to 4.9% recall@5. Comparign to train_seq_pred_extended_vocab.py

No user_id added. 

Train a LoRA adaptor


DDP using all GPUs available.
# Using torchrun (PyTorch >=1.10)
$ torchrun --nproc_per_node=8 train_seq_pred_aligned_vocab.py
"""
...
OUTPUT_MODEL_DIR = config.MODEL_DIR / "train_seq_pred_aligned_vocab"
...
DECAY_STEPS = 1_000     # 3k decay is worse -> needs quick decay
MIN_LR_RATIO = 0.01     # 0.1 overfit, 0.08 overfit, 0.05 overfits, 0.03 overfits a bit. 
...
LOGGING_DIR =  config.RUN_DIR / "train_seq_pred_aligned_vocab" / run_name
...
def main():
    ...
    model, tokenizer = checkpoint_loading.load_model_tokenizer(MODEL_INPUT_DIR)
