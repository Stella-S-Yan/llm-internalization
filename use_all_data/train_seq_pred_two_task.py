"""
Train sequence prediction model using extended and ALOGNED vocabulary. 
Update LoRA parameters only. 

Hard to train, do not see improvement. OOM is do not trucate text sequence.

DDP using all GPUs available.
# Using torchrun (PyTorch >=1.10)
$ torchrun --nproc_per_node=8 train_seq_pred_two_task.py
"""
...
MODEL_SAVE_DIR = config.MODEL_DIR / f"train_seq_pred_two_task"
...
# TEMPLATE_SEQ = """Rule:
#             Each product ID has four hierarchical and categorical levels. 
...
# TEMPLATE_TEXT = """Rule:
#             Each product ID is a quantized representation of a product derived from its text description.
#             It has four hierarchical categorical levels, where earlier levels are more important than later levels.

#             Task:
#             Given a product's text description, predict its full product ID with four hierarchical levels.
...
class SeqDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        prompt = TEMPLATE_SEQ.format(uid=uid, history=input, target=target).strip()
...
class TextDataset(Dataset):
    def __init__(self, tokenizer):
        ...
        self.data = meta_df[["formatted_text", "formatted_sid"]].values.tolist()
    ...
    def __getitem__(self, idx):
        ...
        prompt = TEMPLATE_TEXT.format(description=record[0],
                                      target=record[1]).strip()
...
def evaluate_sequence_recall(
    ...
    for batch in tqdm(eval_loader, desc="Evaluating"):
        ...
        for i in range(batch_size):
            ...
            decoded_outputs = [
                tokenizer.decode(batch_outputs[i, k, prompt_len:].cpu(), skip_special_tokens=True)
    ...
    del inputs, outputs, batch_outputs
    gc.collect()
...
def train(model, tokenizer, train_dataset, eval_dataset, gen_eval_dataset, params):
    ...
    lora_config = LoraConfig(
        r=LORA_RANK,                      # rank
        lora_alpha=LORA_RANK * params.LORA_RATIO,
...
def main():
    ...
    parser.add_argument("--ACCUMULATION_STEPS", type=int, default=2, help="Accumulation steps")
    ...
    run_name = f"lr{Params.LR}_weight_decay{Params.WEIGHT_DECAY}_bs{Params.TRAIN_BATCH_SIZE}_as{Params.ACCUMULATION_STEPS}_warmup_{Params.WARMUP_STEPS}_lora_ratio{Params.LORA_RATIO}_lora_dropout{Params.LORA_DROPOUT}_total_steps{Params.TOTAL_STEPS}"
    Params.LOGGING_DIR =  config.RUN_DIR / "train_seq_pred_two_task" / run_name

    model, tokenizer = load_model_tokenizer()
    old_vocab_size = 128_256
    
    train_seq_dataset = SeqDataset(tokenizer, "train")
    train_text_dataset = TextDataset(tokenizer)

    train_dataset = ConcatDataset([train_seq_dataset, train_text_dataset])
