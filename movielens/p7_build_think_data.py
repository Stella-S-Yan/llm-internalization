...
def do_the_work_parallel(split, model_dir, metadata_path):
    ...
    if split == "train": data_reader = bagz.Reader(config.TRAIN_DATA)
    elif split == "eval": data_reader = bagz.Reader(config.EVAL_DATA)
    elif split == "test": data_reader = bagz.Reader(config.TEST_DATA)
    elif split == "train_eval": data_reader = bagz.Reader(config.TRAIN_EVAL_DATA)
...
def main():
    model_dir = config.MODEL_DIR / f"{config.DATA_SOURCE}_{config.REVIEW_TYPE}_sid_alignment"
    metadata_path = config.ML_SID # Pass the path, not the dataframe
