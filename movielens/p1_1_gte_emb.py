"""
Store embeddings outside of dataframe using multi-GPU SentenceTransformer.
"""


import logging
import numpy as np
import pandas as pd
from sentence_transformers import SentenceTransformer
import torch
import config

logging.basicConfig(level=logging.INFO)
logger = logging.getLogger(__name__)

MODEL_NAME = "thenlper/gte-large"

# ---------------------------------------------------
# Multi-GPU embedding helper
# ---------------------------------------------------
def encode_texts_multi_gpu(texts, model, pool, batch_size=1024):
    """
    texts: List[str]
    returns: np.ndarray [N, D] float32
    """
    embeddings = model.encode(
        texts,
        pool=pool,
        batch_size=batch_size,
        normalize_embeddings=True,
        show_progress_bar=True,
        convert_to_numpy=True
    )
    return embeddings.astype("float32")


# Main embedding pipeline
def gen_embedding():
    # 1️ Load data
    column_names = ['MovieID', 'Title', 'Genre']
    movies_df = pd.read_csv(config.MOVIELES_MOVIES_DATASET, sep='::', names=column_names, engine='python', header=None, encoding='ISO-8859-1')

    # Stable row id (CRITICAL)
    movies_df["row_id"] = movies_df.index.astype("int64")

    logger.info(f"Loaded dataframe: {movies_df.shape}")

    movies_df["formatted_text"] = movies_df["Title"].fillna("") + "  " + movies_df["Genre"].fillna("")

    formatted_text = (
        movies_df["formatted_text"]
        .astype(str)
        .tolist()
    )

    # 3️ Load model + start multi-GPU pool
    model = SentenceTransformer(MODEL_NAME)
    pool = model.start_multi_process_pool()  # uses all visible GPUs

    try:
        # 4️ Encode formatted test
        logger.info("Encoding formatted with GTE")
        dest_embeddings = encode_texts_multi_gpu(
            texts=formatted_text,
            model=model,
            pool=pool
        )

        np.save(
            config.ML_OUTSIDE_EMB + "_gte.npy",
            dest_embeddings
        )

        
    finally:
        model.stop_multi_process_pool(pool)

    # 6️ Save metadata ONLY (no embeddings)
    movies_df.to_parquet(
        config.ML_DF,
        engine="pyarrow",
        compression="zstd", 
        index=False
    )

    logger.info("GTE embedding pipeline finished successfully")


# Entrypoint
if __name__ == "__main__":
    gen_embedding()


# Use the embeddings later
# destination_context

# df = pd.read_parquet(config.LEPARD_DEST_DF)
# dest_emb = np.load(config.LEPARD_OUTSIDE_EMB + "_dest_gte.npy", mmap_mode="r")

# row_id = meta.loc[i, "row_id"]
# vec = dest_emb[row_id]

# Quote

# quote_meta = pd.read_parquet(config.LEPARD_QUOTE_DF)
# quote_emb = np.load(config.LEPARD_OUTSIDE_EMB + "_quote_gte.npy", mmap_mode="r")

# pid_to_idx = dict(zip(quote_meta.passage_id, quote_meta.index))
# vec = quote_emb[pid_to_idx[passage_id]]