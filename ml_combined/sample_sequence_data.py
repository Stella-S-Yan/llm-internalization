...
class UnbiasedSampleSeqDataset(Dataset):
    def __init__(self):
        ...
        for review_type in review_types:
            ...
            for r in records:
                n = len(r['sequence']) - 2
                total += (n - config.MIN_HISTORY_LEN)
                self.users.append((r, review_type, n))
    ...
    def _sample_subsequence(self):
        ...
        # sample a global position
        x = random.randint(0, self.cum_sizes[-1] - 1)
        
        # find which user it belongs to
        i = bisect.bisect_right(self.cum_sizes, x)
        record, review_type, n = self.users[i]

        # compute local end position
        prev = self.cum_sizes[i-1] if i > 0 else 0
        local_end = config.MIN_HISTORY_LEN + (x - prev) + 1

        seq = record['sesquence'][:-2]
        end = local_end

        L = random.randint(config.MAX_HISTORY_LEN, min(config.MAX_HISTORY_LEN, end))
        start = end - L
