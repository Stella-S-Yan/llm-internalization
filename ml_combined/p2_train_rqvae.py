...
def get_data():
    ...
    file_name = os.path.join(config.PROCESSED_DATA_DIR, f"{config.DATA_SOURCE}_outside_emb_gte.npy")
    emb = np.load(str(file_name), mmap_mode="r")
...
def train():
    ...
    checkpoint_dir = os.path.join(config.MODEL_DIR, f"{config.DATA_SOURCE}_Combined_rqvae")
