...
MODEL_SAVE_DIR = config.MODEL_DIR / f"{config.DATA_SOURCE}_combined_sid_alignment"
...
class SIDDataset(Dataset):
    def __init__(self, df):
        ...
        check_idx = 111
        out_prefix = out_prefix = str(os.path.join(config.PROCESSED_DATA_DIR, f"{config.DATA_SOURCE}_llm_emb"))
