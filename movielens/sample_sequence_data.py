...
class SampleSeqDataset(Dataset):
    def __init__(self):
        ...
        self.data = bagz_utils.read_record(config.USER_SEQUENCE)
        
        # meta data
        df = pd.read_parquet(config.ML_SID)
    ...
    def _sample_subsequence(self, record):
        ...
        if n < config.MIN_HISTORY_LEN:
            # Sequence too short, return the whole sequence
            return seq

        # Choose a random subsequence length
        L = random.randint(config.MIN_HISTORY_LEN, min(config.MAX_HISTORY_LEN, n))
